...
def plot_4M_2A_precond(M_list, plot_over_iter_orig_bad, plot_over_iter_orig_good,
    ...
    for i in range(4):
        ...
        plt.subplot(2, 2, i+1)
        l1 = plt.plot(plot_over_iter_orig_bad[i], c='b', label=f"cond(A) = {dict_of_cond[i]['orig bad']: .0e}")
        l2 = plt.plot(plot_over_iter_orig_good[i], c='y', label=f"cond(A) = {dict_of_cond[i]['orig good']: .0e}")
        l3 = plt.plot(plot_over_iter_precond_bad[i], c='purple', label=f"cond(A) = {dict_of_cond[i]['prec bad']: .0e}")
        l4 = plt.plot(plot_over_iter_precond_good[i], c='orange', label=f"cond(A) = {dict_of_cond[i]['prec good']: .0e}")
