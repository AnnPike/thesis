...
for i in range(4):
    ...
    A_tensor = generate_tall_tensor_eigen(m, p, 1, 10**4, funM, invM)
    ...
    A_tensor = generate_tall_tensor_eigen(m, p, 1, 1, funM, invM)
    B = m_prod(A_tensor, X_true, funM, invM)
    X, error = algo.LSQR_mprod(A_tensor, B, funM, invM, iters, X_true=X_true)
    ...
    plt.ylabel('error')
    plt.legend()
plt.suptitle('M prod LSQR, A generated with M and additional application of DCT')
plt.tight_layout()
...
for i in range(4):
    ...
    A_tensor = generate_tensor_eigen(m, p, 1, 10**4, funM, invM)
    ...
    A_tensor = generate_tensor_eigen(m, p, 1, 1, funM, invM)
    B = m_prod(A_tensor, X_true, funM, invM)
    X, error = algo.CG_M_tensor(A_tensor, B, funM, invM, iters, X_true=X_true)
    bound_vector = bond_vector_f(np.arange(iters+1), 2, error[0])
    ...
    plt.ylabel('error')
    plt.legend()
plt.suptitle('M prod conjugate gradient, A generated with M and additional application of DCT')
plt.tight_layout()
